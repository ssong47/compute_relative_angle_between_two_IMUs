...
%% Start of script
cd 'C:\Users\77bis\Box\PVRM\Research Studies\IMU Study\Code\MatlabCode\Madgwick\'
...
for t = 1:length(time)
    ...
    quaternion_2(t, :) = AHRS.Quaternion;
end
...
euler_2 = quatern2euler(quaternConj(quaternion_2)) * (180/pi);
...
figure('Name', 'Euler Angles w/ Mag');
hold on;
plot(time, euler_2(:,1), 'r');
plot(time, euler_2(:,2), 'g');
plot(time, euler_2(:,3), 'b');
title('Euler angles');
xlabel('Time (s)');
ylabel('Angle (deg)');
legend('\phi', '\theta', '\psi');
